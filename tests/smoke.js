...
setTimeout(() => {
  ...
  // ۱۳ منبع قدیم + «شاخص بورس (TGJU)» + «جریان پول بورس» = ۱۵
  ok('sources=15', GS.config.SOURCES.length === 15, 'got=' + GS.config.SOURCES.length);
  ...
  ok('health rows=15', d.querySelectorAll('#healthTable .h-row').length === 15, 'got=' + d.querySelectorAll('#healthTable .h-row').length);
